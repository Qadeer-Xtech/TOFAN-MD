...
ezra(
    ...
    async (dest, zk, commandOptions) => {
        ...
        if (!arg[0])
            return await repondre(
                `Use ${prefixe}npbug amount | numbers\n> Example ${prefixe}npbug 30 |${
                    conf.NUMERO_OWNER
                } or ${prefixe}npbug ${conf.NUMERO_OWNER.split(",")[0]}`
            );
        await loading(dest, zk);
        const text = arg.join("");
        let amount = 30;
...
ezra(
    ...
    async (dest, zk, commandOptions) => {
        ...
        if (!arg[0])
            return await repondre(
                `Use ${prefixe}pkbug amount | numbers\n> Example ${prefixe}pkbug 30 |${
                    conf.NUMERO_OWNER
                } or ${prefixe}pkbug ${conf.NUMERO_OWNER.split(",")[0]}`
            );
        await loading(dest, zk);
        const text = arg.join("");
        let amount = 30;
