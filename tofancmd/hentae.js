...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and TOFAN-MD will extract the text for you.");
    }
...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and TOFAN-MD will generate it.");
    }
    const prompt = args.join(" ");
